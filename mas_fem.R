regg <- m20 %>% 
  ...
  select(gender, trust_all, mf, mas, fem, fem_nat, pid2, rep, literal, lit2, fem, educ = q105, income = q108, defy2, attend, white, age)


reg1 <- glm(defy2 ~ fem_nat*gender + literal + trust_all + attend + white + rep +  income + educ, data = regg, family = "binomial")
...
gg +
  ...
  labs(x = "How Masculine Do You Feel?", y = "Church Should Defy Govt. Order", title = "Interaction of Masculine Attributes and Gender on Defying Govt. Orders") +
  ggsave("D://women_covid/images/defy_interact_mas.png", type = "cairo-png", width =7)
